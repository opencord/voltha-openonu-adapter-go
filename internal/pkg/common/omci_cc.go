...
func (oo *OmciCC) sendQueuedLowPrioRequests(ctx context.Context) error {
	...
	for oo.lowPrioTxQueue.Len() > 0 {
		...
		if aOmciTxReq.OnuSwWindow != nil {
			if err := oo.sendOnuSwSectionsOfWindow(ctx, aOmciTxReq.OnuSwWindow); err != nil {
				oo.mutexLowPrioTxQueue.Unlock()
				return err
			}
		} else {
...
func (oo *OmciCC) PrepareOnuSectionsOfWindow(ctx context.Context, aTimeout int, highPrio bool,
	...
	if aAckRequest > 0 {
		...
		omciTxReq = OmciTransferStructure{
			timeout:        timeout,
			retries:        CDefaultRetries,
			highPrio:       highPrio,
			withFramePrint: false,
			cbPair:         omciRxCallbackPair,
		...
		return omciTxReq, fmt.Errorf("no timeout present for last section of window")
...
func (oo *OmciCC) SendOnuSwSectionsWindowWithRxSupervision(ctx context.Context, aOmciTxRequest OmciTransferStructure, aTimeout int) {
...
func (oo *OmciCC) sendOnuSwSectionsOfWindow(ctx context.Context, omciMsgsPerWindow *ia.OmciMessages) error {
