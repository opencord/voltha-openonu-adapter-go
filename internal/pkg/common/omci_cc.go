...
func (oo *OmciCC) PrepareOnuSectionsOfWindow(ctx context.Context, aTimeout int, highPrio bool,
	rxChan chan Message, aImageMeID uint16, aAckRequest uint8, aDownloadSectionNo uint8, aSection []byte, aPrint bool, omuTxSecPerWindow *[]*OmciTransferStructure) error {
	tid := oo.GetNextTid(highPrio)
	...
	*omuTxSecPerWindow = append(*omuTxSecPerWindow, omciTxReq)

	if aAckRequest > 0 {

		lastSectionOfWindow := (*omuTxSecPerWindow)[len(*omuTxSecPerWindow)-1]

		// only the last section should have a timeout as an ack is required only for the last section of the window
		if lastSectionOfWindow.timeout != 0 {
...
func (oo *OmciCC) SendOnuSwSectionsWindowWithRxSupervision(ctx context.Context, aOmciTxRequests []*OmciTransferStructure, aTimeout int) {
...
loop:
	for retryCounter <= retries {
		go oo.sendOnuSwSectionsOfWindow(ctx, aOmciTxRequests)
...
func (oo *OmciCC) sendOnuSwSectionsOfWindow(ctx context.Context, omciTxRequests []*OmciTransferStructure) {
	...
	sendErr := oo.pBaseDeviceHandler.SendOnuSwSectionsOfWindow(ctx, oo.pBaseDeviceHandler.GetProxyAddress().AdapterEndpoint, omciMsg)
	if sendErr != nil {
