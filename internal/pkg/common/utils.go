...
func IsValidTechProfileInstance(ctx context.Context, techProfMsg inter_adapter.TechProfileDownloadMessage) error {
	...
	if tp.NumGemPorts == 0 {
		return fmt.Errorf("no GEM ports configured for the Tp Instance")
	}
	if len(tp.UpstreamGemPortAttributeList) == 0 {
		return fmt.Errorf("no Upstream GEM port attributes configured for the Tp Instance")
	}
	if len(tp.DownstreamGemPortAttributeList) == 0 {
		return fmt.Errorf("no Downstream GEM port attributes configured for the Tp Instance")
	}

	if tp.GetUsScheduler() == nil {
		return fmt.Errorf("no upstream scheduler configured for the Tp Instance")
	}
