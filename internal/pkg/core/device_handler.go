...
func (dh *deviceHandler) createInterface(ctx context.Context, onuind *oop.OnuIndication) error {
	...
	if dh.IsReconciling() && (dh.device.OperStatus == common.OperStatus_ACTIVATING) {

		logger.Debugw(ctx, "Reconciling an ONU in previously activating state ,stop reconciling ,  MIB reset and resume normal start up",
			log.Fields{"device-id": dh.DeviceID})
		dh.stopReconciling(ctx, true, cWaitReconcileFlowNoActivity)
