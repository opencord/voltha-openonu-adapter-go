...
func (mm *OnuMetricsManager) updateControlBlockEthernetFrameExtendedPMMe(ctx context.Context, upstream bool, entityID uint16, meName string) (error, extension.GetValueResponse_ErrorReason) {
...
func (mm *OnuMetricsManager) resetEthernetFrameExtendedPMCounters(ctx context.Context,
	...
	for entityID, _ := range upstreamEntityMap {
		err, errReason := mm.updateControlBlockEthernetFrameExtendedPMMe(ctx, true, entityID, className)
		if err != nil {
	...
	for entityID, _ := range downstreamEntityMap {
		err, errReason := mm.updateControlBlockEthernetFrameExtendedPMMe(ctx, false, entityID, className)
		if err != nil {
			return err, errReason
		}
	}
