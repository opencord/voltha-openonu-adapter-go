...
func (mm *OnuMetricsManager) updateControlBlockEthernetFrameExtendedPMMe(ctx context.Context, upstream bool, entityID uint16, meName string) (extension.GetValueResponse_ErrorReason, error) {
...
func (mm *OnuMetricsManager) resetEthernetFrameExtendedPMCounters(ctx context.Context,
	...
	for entityID := range upstreamEntityMap {
		errReason, err := mm.updateControlBlockEthernetFrameExtendedPMMe(ctx, true, entityID, className)
		if err != nil {
	...
	for entityID := range downstreamEntityMap {
		errReason, err := mm.updateControlBlockEthernetFrameExtendedPMMe(ctx, false, entityID, className)
		if err != nil {
			return errReason, err
		}
	}
