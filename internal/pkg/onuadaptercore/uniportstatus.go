...
func (portStatus *UniPortStatus) getUniPortStatus(ctx context.Context, uniIdx uint32) *extension.SingleGetValueResponse {
	...
	//uniPort, ok := portStatus.pOmiCC.pBaseDeviceHandler.uniEntityMap[uniIdx]
	for _, uniPort := range portStatus.pOmiCC.pBaseDeviceHandler.uniEntityMap {
		...
		if uniPort.uniID == uint8(uniIdx) && uniPort.portType == uniPPTP {
			...
			meInstance = portStatus.pOmiCC.sendGetPptpEthUni(ctx, uniPort.entityID, ConstDefaultOmciTimeout, true, requestedAttributes, portStatus.omciRespChn)
	...
	return sendUniStatusErrResponse(extension.GetValueResponse_INVALID_PORT_TYPE)
}
...
func (portStatus *UniPortStatus) waitforgetUniPortStatus(ctx context.Context, apMeInstance *me.ManagedEntity) *extension.SingleGetValueResponse {
	...
	case <-ctx.Done():
		...
		return sendUniStatusErrResponse(extension.GetValueResponse_INTERNAL_ERROR)
	case <-time.After(uniStatusTimeout * time.Second):
		logger.Errorf(ctx, "waitforgetUniPortStatus  timeout")
		return sendUniStatusErrResponse(extension.GetValueResponse_TIMEOUT)
...
func sendUniStatusErrResponse(reason extension.GetValueResponse_ErrorReason) *extension.SingleGetValueResponse {
