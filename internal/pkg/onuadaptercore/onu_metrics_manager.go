...
loop:
	for _, anigInstID := range anigInstKeys {
		...
		if meInstance := mm.pDeviceHandler.pOnuOmciDevice.PDevOmciCC.sendGetMetricsMe(ctx, me.AniGClassID, anigInstID, requestedAttributes, ConstDefaultOmciTimeout, true); meInstance != nil {
			select {
...
loop1:
	for _, unigInstID := range unigInstKeys {
		...
		if meInstance := mm.pDeviceHandler.pOnuOmciDevice.PDevOmciCC.sendGetMetricsMe(ctx, me.UniGClassID, unigInstID, requestedAttributes, ConstDefaultOmciTimeout, true); meInstance != nil {
...
loop2:
	for _, pptpInstID := range pptpInstKeys {
		...
		if meInstance := mm.pDeviceHandler.pOnuOmciDevice.PDevOmciCC.sendGetMetricsMe(ctx, me.PhysicalPathTerminationPointEthernetUniClassID, pptpInstID, requestedAttributes, ConstDefaultOmciTimeout, true); meInstance != nil {
			// Wait for metrics or timeout
			select {
			case meAttributes = <-mm.uniStatusMetricsChan:
...
loop3:
	for _, veipInstID := range veipInstKeys {
		...
		if meInstance := mm.pDeviceHandler.pOnuOmciDevice.PDevOmciCC.sendGetMetricsMe(ctx, me.VirtualEthernetInterfacePointClassID, veipInstID, requestedAttributes, ConstDefaultOmciTimeout, true); meInstance != nil {
			// Wait for metrics or timeout
			select {
			case meAttributes = <-mm.uniStatusMetricsChan:
...
func (mm *onuMetricsManager) processOmciMessages(ctx context.Context) {
	...
	// The processOmciMessages routine will get stopped startCollector routine (in device_handler.go)
...
func (mm *onuMetricsManager) flushMetricCollectionChannels(ctx context.Context) {
	...
	select {
	case <-mm.commMetricsChan:
		logger.Debug(ctx, "flushed optocal metrics channel")
	default:
	}
