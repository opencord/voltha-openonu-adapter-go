...
type onuMetricsManager struct {
	...
	ethernetFrameExtendedPMGetCommands			  int
	ethernetFrameExtendedPMGetSeqNo				  int
}
...
func (mm *onuMetricsManager) GetSequenceNumberForExtendedPM() int {
	...
	if meInstance != nil {
		...
		if val, ok := meAttributes["MeTypeTable"]; ok && val != nil {
			...
			if okay {
				...
				mm.ethernetFrameExtendedPMGetCommands = numCommands
