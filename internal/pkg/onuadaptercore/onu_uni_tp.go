...
func (onuTP *OnuUniTechProf) restoreFromOnuTpPathKvStore(ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	logger.Debugw("restore ONU/TP-data from KVStore", log.Fields{"deviceID": onuTP.deviceID})

	if onuTP.onuKVStore == nil {
		logger.Debugw("onuKVStore not set - abort", log.Fields{"device-id": onuTP.deviceID})
		onuTP.procResult = errors.New("ONU/TP-data restore aborted: onuKVStore not set")
		return
	}
	var processingStep uint8 = 1 // used to synchronize the different processing steps with chTpProcessingStep

	go onuTP.restorePersistentData(ctx, processingStep)
	if !onuTP.waitForTimeoutOrCompletion(ctx, processingStep) {
		//timeout or error detected
		logger.Debugw("ONU/TP-data not read - abort", log.Fields{"device-id": onuTP.deviceID})
		onuTP.procResult = errors.New("ONU/TP-data restore aborted: during reading process")
		return
	}
}

func (onuTP *OnuUniTechProf) deleteOnuTpPathKvStore(ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()

	if onuTP.onuKVStore == nil {
		logger.Debugw("onuKVStore not set - abort", log.Fields{"device-id": onuTP.deviceID})
		onuTP.procResult = errors.New("ONU/TP-data delete aborted: onuKVStore not set")
		return
	}
	var processingStep uint8 = 1 // used to synchronize the different processing steps with chTpProcessingStep
	go onuTP.deletePersistentData(ctx, processingStep)
	if !onuTP.waitForTimeoutOrCompletion(ctx, processingStep) {
		//timeout or error detected
		logger.Debugw("ONU/TP-data not deleted - abort", log.Fields{"device-id": onuTP.deviceID})
		onuTP.procResult = errors.New("ONU/TP-data delete aborted: during deleting process")
		return
	}
}
...
func (onuTP *OnuUniTechProf) storePersistentData(ctx context.Context, aProcessingStep uint8) {
	...
	err = onuTP.onuKVStore.Put(context.TODO(), onuTP.onuKVStorePath, Value)
	if err != nil {
...
func (onuTP *OnuUniTechProf) restorePersistentData(ctx context.Context, aProcessingStep uint8) {

	onuTP.mapUniTpPath = make(map[uint32]string)
	onuTP.sOnuPersistentData = onuPersistentData{0, 0, "", "", "", make([]uniPersData, 0)}

	Value, err := onuTP.onuKVStore.Get(context.TODO(), onuTP.onuKVStorePath)
	if err == nil {
		if Value != nil {
...
func (onuTP *OnuUniTechProf) deletePersistentData(ctx context.Context, aProcessingStep uint8) {

	logger.Debugw("delete ONU/TP-data in KVStore", log.Fields{"deviceID": onuTP.deviceID})
	err := onuTP.onuKVStore.Delete(context.TODO(), onuTP.onuKVStorePath)
	if err != nil {
