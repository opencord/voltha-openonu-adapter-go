...
func NewOnuUniTechProf(ctx context.Context, aDeviceID string, aDeviceHandler *DeviceHandler) *OnuUniTechProf {
	...
	if onuTP.techProfileKVStore == nil {
		logger.Errorw("Failed to setup techProfileKVStore", log.Fields{"deviceID": aDeviceID})
	}
...
func (onuTP *OnuUniTechProf) configureUniTp(aUniID uint32, aPathString string, wg *sync.WaitGroup) {
	...
	if err == nil {
		if Value != nil {
			logger.Debugf("tech-profile read: Key: %s, Value: %s  %s", Value.Key, Value.Value)
			tpTmpBytes, _ := kvstore.ToByte(Value.Value)

			var tpInst tp.TechProfile
			if err = json.Unmarshal(tpTmpBytes, &tpInst); err != nil {
				logger.Error("TechProf - Failed to unmarshal tech-profile into tpInst", log.Fields{"error": err, "device-id": onuTP.deviceID})
			} else {
		...
		logger.Errorf("Get from kvstore operation failed for path %s", aPathString)
	}
}
