...
func (onuTP *onuUniTechProf) readAniSideConfigFromTechProfile(
	...
	// Lock to protect concurrent critical state access
	onuTP.mutexTPState.Lock()
	defer onuTP.mutexTPState.Unlock()
	...
	if _, existTP := onuTP.mapUniTpIndication[uniTPKey]; existTP {
		...
		if uint8(profID) == onuTP.mapUniTpIndication[uniTPKey].techProfileID {
	...
	onuTP.mapUniTpIndication[uniTPKey].techProfileID = uint8(profID)
