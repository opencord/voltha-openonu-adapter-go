...
func (onuTP *onuUniTechProf) readAniSideConfigFromTechProfile(
	...
	for pos, downstreamContent := range tpInst.DownstreamGemPortAttributeList {
		...
		if pos == 0 {
			//at least one downstream GemPort should always exist (else traffic profile makes no sense)
			loGemPortRead = true
		} else {
			//for all further GemPorts we need to extend the mapGemPortParams
			onuTP.mapPonAniConfig[uniTPKey].mapGemPortParams[uint16(pos)] = &gemPortParamStruct{}
		...
		if isMulticast {
			...
			onuTP.mapPonAniConfig[uniTPKey].mapGemPortParams[uint16(pos)].isMulticast = isMulticast
			onuTP.mapPonAniConfig[uniTPKey].mapGemPortParams[uint16(pos)].multicastGemPortID = uint16(downstreamContent.McastGemID)
			onuTP.mapPonAniConfig[uniTPKey].mapGemPortParams[uint16(pos)].staticACL = downstreamContent.SControlList
			onuTP.mapPonAniConfig[uniTPKey].mapGemPortParams[uint16(pos)].dynamicACL = downstreamContent.DControlList
