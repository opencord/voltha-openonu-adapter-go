...
func newDeviceHandler(ctx context.Context, cp adapterif.CoreProxy, ap adapterif.AdapterProxy, ep eventif.EventProxy, device *voltha.Device, adapter *OpenONUAC) *deviceHandler {
	...
	dh.flowCbChan = make([]chan FlowCb, maxUnisPerOnu)
	dh.stopFlowMonitoringRoutine = make([]chan bool, maxUnisPerOnu)
	dh.isFlowMonitoringRoutineActive = make([]bool, maxUnisPerOnu)
	for i := 0; i < maxUnisPerOnu; i++ {
...
func (dh *deviceHandler) resetFsms(ctx context.Context, includingMibSyncFsm bool) error {
	...
	/* // We want flow deletes to be processed on onu down, so do not stop flow monitoring routines
	for _, uniPort := range dh.uniEntityMap {
		...
		if (1<<uniPort.uniID)&dh.pOpenOnuAc.config.UniPortMask == (1 << uniPort.uniID) {
			if dh.getFlowMonitoringIsRunning(uniPort.uniID) {
				dh.stopFlowMonitoringRoutine[uniPort.uniID] <- true
			}
		}
	}
	*/
