...
func (dh *deviceHandler) doStateInit(ctx context.Context, e *fsm.Event) {
	...
	if !dh.reconciling {
		...
		logger.Infow(ctx, "DeviceReasonUpdate", log.Fields{"device-id": dh.deviceID, "reason": deviceReasonMap[dh.deviceReason]})
		_ = dh.deviceReasonUpdate(ctx, drActivatingOnu, true)
	} else {
