...
func newDeviceHandler(ctx context.Context, cp adapterif.CoreProxy, ap adapterif.AdapterProxy, ep eventif.EventProxy, device *voltha.Device, adapter *OpenONUAC) *deviceHandler {
	...
	dh.flowCbChan = make(chan FlowCb, dh.pOpenOnuAc.maxConcurrentFlowsPerOnu)
		...
		for _, flowItem := range apOfFlowChanges.ToRemove.Items {
			...
			if flowInPort == uint32(of.OfpPortNo_OFPP_INVALID) {
				...
				if dh.getFlowMonitoringIsRunning() {
					...
					startTime := time.Now()
					logger.Infow(ctx, "process-flow", log.Fields{"device-id": dh.deviceID, "addFlow": false})
					...
					retError = <-respChan
					logger.Infow(ctx, "process-flow--received-resp", log.Fields{"device-id": dh.deviceID, "err": retError, "totalTimeSeconds": time.Since(startTime).Seconds()})
					if retError != nil {
		...
		for _, flowItem := range apOfFlowChanges.ToAdd.Items {
			...
			if flowInPort == uint32(of.OfpPortNo_OFPP_INVALID) {
				...
				if dh.getFlowMonitoringIsRunning() {
					...
					// Step4 : Return error value
					startTime := time.Now()
					logger.Infow(ctx, "process-flow", log.Fields{"device-id": dh.deviceID, "addFlow": true})
					respChan := make(chan error)
					flowCb := FlowCb{
						ctx:          ctx,
						addFlow:      true,
...
func (dh *deviceHandler) perOnuFlowHandlerRoutine() {
	...
	dh.setFlowMonitoringIsRunning(true)
	for {
		select {
