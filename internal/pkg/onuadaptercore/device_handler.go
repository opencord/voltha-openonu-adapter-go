...
func (dh *deviceHandler) startCollector(ctx context.Context) {
	...
	for {
		...
		case <-dh.stopCollector:
			...
			// Stop the L2 PM FSM
			go func() {
				if err := dh.pOnuMetricsMgr.pL2PMFsm.Event(l2PmEventStop); err != nil {
					logger.Errorw(ctx, "error calling event", log.Fields{"device-id": dh.deviceID, "err": err})
				}
			}()
			// We also need to reset the activeL2Pms slice so that the L2 PM MEs are re-created once the ONU is back up.
			dh.pOnuMetricsMgr.resetActiveL2PMList()
...
func (dh *deviceHandler) l2PmFsmInIdleState(ctx context.Context, idleState string) bool {
	if dh.pOnuMetricsMgr != nil && dh.pOnuMetricsMgr.pL2PMFsm != nil {
		return dh.isFsmInState(ctx, dh.pOnuMetricsMgr.pL2PMFsm, idleState)
	}
