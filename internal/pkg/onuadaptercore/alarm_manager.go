...
// alarmBitMapSize is the size of alarm bitmap in bytes, since omci.AlarmBitmapSize is in bits, converting in byte here.
...
func (am *onuAlarmManager) processAlarmData(ctx context.Context, msg *omci.AlarmNotificationMsg) error {
	...
	meAlarmMap := make(map[uint8]string) //just dummy for now
	...
	for alarmNo, _ := range meAlarmMap {
...
func (am *onuAlarmManager) sendAlarm(ctx context.Context, classID me.ClassID, instanceID uint16, alarm uint8, raised bool) {
	...
	event_details, err := getDeviceEventData(ctx, classID, alarm)
	if err != nil {
