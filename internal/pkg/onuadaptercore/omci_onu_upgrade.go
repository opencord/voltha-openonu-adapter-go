...
func (oFsm *OnuUpgradeFsm) enterPreparingDL(ctx context.Context, e *fsm.Event) {
	...
	//copy file content to buffer, omitting last EOF entry
	oFsm.imageBuffer = make([]byte, fileLen-1)
	if oFsm.useAPIVersion43 {
	...
	case omci.DownloadSectionResponseType:
		{
			...
			if msgObj.EntityInstance == oFsm.inactiveImageMeID {
				...
				if oFsm.nextDownloadWindow >= oFsm.noOfWindows {
					...
					//CRC computation for all data bytes of the file (not using EOF byte)
					oFsm.imageCRC = crc32a.Checksum(oFsm.imageBuffer[:int(oFsm.origImageLength-1)]) //store internal for multiple usage
