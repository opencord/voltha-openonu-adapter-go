		...
		Response: &extension.GetValueResponse{
			...
			Response: &extension.GetValueResponse_OnuOpticalInfo{
				OnuOpticalInfo: &extension.GetOnuPonOpticalInfoResponse{
					...
					PowerFeedVoltage: float32(selfTestCb.twosComplementToSignedInt16(msgObj.PowerFeedVoltage)) * 0.02,
					// OMCI representation is Decibel-microwatts, 2s compliment, 0.002dB resolution
					ReceivedOpticalPower: float32(selfTestCb.twosComplementToSignedInt16(msgObj.ReceivedOpticalPower)) * 0.002,
					// OMCI representation is Decibel-microwatts, 2s compliment, 0.002dB resolution
					MeanOpticalLaunchPower: float32(selfTestCb.twosComplementToSignedInt16(msgObj.MeanOpticalLaunch)) * 0.002,
					// OMCI representation is unsigned int, 2uA resolution
					LaserBiasCurrent: float32(msgObj.LaserBiasCurrent) * 0.000002,
					// OMCI representation is 2s complement, 1/256 degree Celsius resolution
					Temperature: float32(selfTestCb.twosComplementToSignedInt16(msgObj.Temperature)) / 256.0,
				},
			},
		},
	}
