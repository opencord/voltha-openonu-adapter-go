...
func (selfTestCb *selfTestControlBlock) triggerFsmEvent(p_selfTestFsm *AdapterFsm, event string, args ...generated.ClassID) {
	go func() {
		if len(args) > 0 {
			_ = p_selfTestFsm.pFsm.Event(event, args[0])
		} else {
			_ = p_selfTestFsm.pFsm.Event(event)
		}
	}()
}
			...
			Response: &extension.GetValueResponse{
				...
				Response: &extension.GetValueResponse_OnuOpticalInfo{
					OnuOpticalInfo: &extension.GetOnuPonOpticalInfoResponse{
						...
						PowerFeedVoltage: int32(float32(selfTestCb.twosComplementToSignedInt16(msgObj.PowerFeedVoltage)) * 0.02),
						// OMCI representation is Decibel-microwatts, 2s compliment, 0.002dB resolution
						ReceivedOpticalPower: int32(float32(selfTestCb.twosComplementToSignedInt16(msgObj.ReceivedOpticalPower)) * 0.002),
						// OMCI representation is Decibel-microwatts, 2s compliment, 0.002dB resolution
						MeanOpticalLaunchPower: int32(float32(selfTestCb.twosComplementToSignedInt16(msgObj.MeanOpticalLaunch)) * 0.002),
						// OMCI representation is unsigned int, 2uA resolution
						LaserBiasCurrent: int32(float32(msgObj.LaserBiasCurrent) * 0.000002),
						// OMCI representation is 2s complement, 1/256 degree Celsius resolution
						Temperature: int32(float32(selfTestCb.twosComplementToSignedInt16(msgObj.Temperature)) / 256.0),
					},
				},
			},
		}
...
func (selfTestCb *selfTestControlBlock) twosComplementToSignedInt16(val uint16) int16 {
