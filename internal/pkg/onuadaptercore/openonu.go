...
func (oo *OpenONUAC) AdoptDevice(ctx context.Context, device *voltha.Device) (*empty.Empty, error) {
	...
	if handler = oo.getDeviceHandler(ctx, device.Id, false); handler == nil {
		...
		if err := oo.setupParentInterAdapterClient(ctx, device); err != nil {
...
func (oo *OpenONUAC) setupParentInterAdapterClient(ctx context.Context, childDevice *voltha.Device) error {
	oo.lockParentAdapterClients.Lock()
	defer oo.lockParentAdapterClients.Unlock()
	if _, ok := oo.parentAdapterClients[childDevice.ProxyAddress.Endpoint]; ok {
		return nil
	}

	childClient, err := vgrpc.NewClient(childDevice.ProxyAddress.Endpoint,
		oo.oltAdapterRestarted,
		vgrpc.ActivityCheck(true),
		vgrpc.ActivityProbeInterval(oo.config.LiveProbeInterval))

	if err != nil {
		return err
	}

	oo.parentAdapterClients[childDevice.ProxyAddress.Endpoint] = childClient

	go oo.parentAdapterClients[childDevice.ProxyAddress.Endpoint].Start(log.WithSpanFromContext(context.TODO(), ctx), setAndTestAdapterServiceHandler)
	...
	for {
		client, err := oo.parentAdapterClients[childDevice.ProxyAddress.Endpoint].GetOltInterAdapterServiceClient()
		if err == nil && client != nil {
			logger.Infow(subCtx, "connected-to-parent-adapter", log.Fields{"parent-endpoint": childDevice.ProxyAddress.Endpoint})
			break
		}
		logger.Warnw(subCtx, "connection-to-parent-adapter-not-ready", log.Fields{"error": err, "parent-endpoint": childDevice.ProxyAddress.Endpoint})
		// Backoff
		if err = backoff.Backoff(subCtx); err != nil {
			logger.Errorw(subCtx, "received-error-on-backoff", log.Fields{"error": err, "parent-endpoint": childDevice.ProxyAddress.Endpoint})
			break
		}
	}
	return nil
