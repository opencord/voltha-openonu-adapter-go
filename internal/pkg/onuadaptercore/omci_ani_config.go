...
func (oFsm *uniPonAniConfigFsm) prepareAndEnterConfigState(ctx context.Context, aPAFsm *AdapterFsm) {
	if aPAFsm != nil && aPAFsm.pFsm != nil {
		...
		//The empty TCONT condition is "AllocId == 0xFF || AllocId == 0xFFFF"
		masks := make([]ordered_map.KVPair, 2)
		masks[0] = ordered_map.KVPair{Key: "AllocId", Value: uint16(0xFF)} //This is added for some vendor compatibility.
		masks[1] = ordered_map.KVPair{Key: "AllocId", Value: uint16(unusedTcontAllocID)}
		...
		logger.Infow(ctx, "Used Tcont Instance Id", log.Fields{"Tcont-inst-id": oFsm.tcont0ID,
...
func (oFsm *uniPonAniConfigFsm) enterResettingTcont(ctx context.Context, e *fsm.Event) {
	...
	attribs := me.AttributeValueMap{
		"AllocId": unusedTcontAllocID,
	}
	oFsm.pOnuDB.PutMe(ctx, me.TContClassID, oFsm.tcont0ID, attribs)
	oFsm.pLastTxMeInstance = meInstance
	oFsm.mutexPLastTxMeInstance.Unlock()
