...
func (oFsm *uniPonAniConfigFsm) enterSettingDot1PMapper(ctx context.Context, e *fsm.Event) {
	...
	meInstance := oFsm.pOmciCC.sendSetDot1PMapperVar(log.WithSpanFromContext(context.TODO(), ctx), ConstDefaultOmciTimeout, true,
		oFsm.pAdaptFsm.commChan, meParams)
	//accept also nil as (error) return value for writing to LastTx
	//  - this avoids misinterpretation of new received OMCI messages
	oFsm.pLastTxMeInstance = meInstance
...
func (oFsm *uniPonAniConfigFsm) performCreatingGemIWs(ctx context.Context) {
	...
	for gemIndex, gemPortAttribs := range oFsm.gemPortAttribsSlice {
		...
		meInstance := oFsm.pOmciCC.sendCreateGemIWTPVar(log.WithSpanFromContext(context.TODO(), ctx), ConstDefaultOmciTimeout, true,
			oFsm.pAdaptFsm.commChan, meParams)
		//accept also nil as (error) return value for writing to LastTx
		//  - this avoids misinterpretation of new received OMCI messages
		oFsm.pLastTxMeInstance = meInstance
