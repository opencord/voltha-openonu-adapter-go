...
func newUniPonAniConfigFsm(apDevOmciCC *omciCC, apUniPort *onuUniPort, apUniTechProf *onuUniTechProf,
	...
	instFsm.fsmKey = uniTP{uniID: apUniPort.uniID, tpID: aTechProfileID}
...
func (oFsm *uniPonAniConfigFsm) prepareAndEnterConfigState(aPAFsm *AdapterFsm) {
	if aPAFsm != nil && aPAFsm.pFsm != nil {
		...
		*/
		...
		oFsm.alloc0ID = oFsm.pUniTechProf.mapPonAniConfig[oFsm.fsmKey].tcontParams.allocID
		mapGemPortParams := oFsm.pUniTechProf.mapPonAniConfig[oFsm.fsmKey].mapGemPortParams
...
func (oFsm *uniPonAniConfigFsm) enterRemovingGemIW(e *fsm.Event) {
	...
	oFsm.pUniTechProf.mutexTPState.Lock()
	loGemPortID := (*(oFsm.pUniTechProf.mapRemoveGemEntry[oFsm.fsmKey])).gemPortID
	oFsm.pUniTechProf.mutexTPState.Unlock()
...
func (oFsm *uniPonAniConfigFsm) handleOmciAniConfigSetResponseMessage(msg OmciMessage) {
	...
	if msgObj.EntityClass == oFsm.pLastTxMeInstance.GetClassID() &&
		...
		case "TCont":
			{ // let the FSM proceed ...
				if 0 == oFsm.requestEventOffset { //from TCont config request
...
func (oFsm *uniPonAniConfigFsm) handleOmciAniConfigDeleteResponseMessage(msg OmciMessage) {
	...
	if msgObj.Result != me.Success {
		logger.Errorw("UniPonAniConfigFsm - Omci DeleteResponse Error - later: drive FSM to abort state ?",
			log.Fields{"device-id": oFsm.deviceID, "Error": msgObj.Result})
		// possibly force FSM into abort or ignore some errors for some messages? store error for mgmt display?
		return
	}
	if msgObj.EntityClass == oFsm.pLastTxMeInstance.GetClassID() &&
		msgObj.EntityInstance == oFsm.pLastTxMeInstance.GetEntityID() {
		//remove ME from DB //TODO??? obviously the Python code does not store the config ...
