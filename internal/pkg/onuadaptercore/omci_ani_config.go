...
func (oFsm *uniPonAniConfigFsm) prepareAndEnterConfigState(ctx context.Context, aPAFsm *AdapterFsm) {
	if aPAFsm != nil && aPAFsm.pFsm != nil {
		...
		masks := make([]interface{}, 2)
		masks[0] = 0xFF
		masks[1] = 0xFFFF

		tcontInstID, tcontAlreadyExist, err := oFsm.pOnuDB.AllocateMeInstance(ctx, me.TContClassID, requestedAttributes, "AllocId", masks...)
		if err != nil {
...
func (oFsm *uniPonAniConfigFsm) handleOmciAniConfigSetResponseMessage(ctx context.Context, msg OmciMessage) {
	...
	if msgObj.Result != me.Success {
		...
		//doing it here may casue a data inconsistancy. To fix this problem we need to think on running
