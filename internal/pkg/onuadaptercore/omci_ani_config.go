	...
	if aPAFsm != nil && aPAFsm.pFsm != nil {
		...
		*/
		...
		for _, gemEntry := range mapGemPortParams {
			...
			if isMulticast, err := strconv.ParseBool(gemEntry.isMulticast); err != nil {
				loGemPortAttribs.isMulticast = isMulticast
				...
				logger.Errorw("Can't parse isMulticast boolean flag", log.Fields{"device-id": oFsm.deviceID,
					"isMulticast": gemEntry.isMulticast, "error": err})
			}
