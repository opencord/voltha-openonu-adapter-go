...
func (oo *OmciCC) sendReboot(ctx context.Context, timeout int, highPrio bool, onuDeviceEntry *OnuDeviceEntry, responseChannel chan Message) error {
	...
	err = onuDeviceEntry.waitForRebootResponse(responseChannel)
	if err != nil {
		logger.Error("InitialBridgeSetup failed at MBSP, aborting ONU Reboot!")
