...
func (oo *omciCC) sendQueuedHighPrioRequests(ctx context.Context) error {
	...
	for oo.highPrioTxQueue.Len() > 0 {
		...
		if err := oo.sendInterAdapterOmciMessage(ctx, queueElement.Value.(omciTransferStructure)); err != nil {
			return err
		}
...
func (oo *omciCC) sendQueuedLowPrioRequests(ctx context.Context) error {
	...
	for oo.lowPrioTxQueue.Len() > 0 {
		...
		if oo.highPrioTxQueue.Len() > 0 {
			logger.Debugw(ctx, "interrupt low prio send loop for high prio request", log.Fields{"device-id": oo.deviceID})
			oo.mutexLowPrioTxQueue.Unlock()
			if err := oo.sendQueuedHighPrioRequests(ctx); err != nil {
				return err
			}
...
func (oo *omciCC) sendInterAdapterOmciMessage(ctx context.Context, omciTxRequest omciTransferStructure) error {
