...
func (oo *omciCC) stop(ctx context.Context) error {
	logger.Debugw(ctx, "omciCC-stopping", log.Fields{"for device-id": oo.deviceID})
...
func (oo *omciCC) receiveMessage(ctx context.Context, rxMsg []byte) error {
	...
	if packet == nil {
		logger.Errorw(ctx, "omci-message could not be decoded", log.Fields{"deviceID": oo.deviceID})
		return fmt.Errorf("could not decode rxMsg as OMCI %s", oo.deviceID)
	}
	omciLayer := packet.Layer(omci.LayerTypeOMCI)
	if omciLayer == nil {
		logger.Errorw(ctx, "omci-message could not decode omci layer", log.Fields{"deviceID": oo.deviceID})
		return fmt.Errorf("could not decode omci layer %s", oo.deviceID)
	}
	omciMsg, ok := omciLayer.(*omci.OMCI)
	if !ok {
		logger.Errorw(ctx, "omci-message could not assign omci layer", log.Fields{"deviceID": oo.deviceID})
	...
	if ok && rxCallbackEntry.cbFunction != nil {
		...
		oo.mutexRxSchedMap.Unlock()
		logger.Errorw(ctx, "omci-message-response for not registered transCorrId", log.Fields{"deviceID": oo.deviceID})
...
func (oo *omciCC) sendNextRequest(ctx context.Context) error {
	...
	for oo.txQueue.Len() > 0 {
		...
		*/
		...
		logger.Debugw(ctx, "omci-message-sending", log.Fields{"fromDeviceType": oo.pBaseDeviceHandler.DeviceType,
		...
		logger.Debugw(ctx, "omci-message-to-send:",
