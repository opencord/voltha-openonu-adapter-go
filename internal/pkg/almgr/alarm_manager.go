...
func (am *OnuAlarmManager) processAlarm(ctx context.Context, omciAlarmMessage *omci.AlarmNotificationMsg, failureTransition func()) error {
	// VOL-5223:
	// When some of the vendor ONTs report unconfigured MEs, such as the VoipConfigDataME,
	// (For instance, during testing, the ONT has responded with VoipConfigDataME Alarm Bit Map
	// when the ONT is not configured for the voice path),
	// the ONU-G ME currently doesn't have the defined Alarm Bit Map.
	// Reporting alarms on that ME causes AlarmMgr to get into a loop state.
	// The MEs as created by OLT as per G984.4 standard are not part of ONU DB,
	// and hence the Alarms reported are not handled for these MEs.
