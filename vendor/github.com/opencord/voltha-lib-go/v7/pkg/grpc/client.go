...
// func getKeepAliveStream(ctx context.Context, service interface{}) (
// 	adapter_service.AdapterService_GetHealthStatusClient,
// 	core_service.CoreService_GetHealthStatusClient,
// 	onu_inter_adapter_service.OnuInterAdapterService_GetHealthStatusClient,
// 	olt_inter_adapter_service.OltInterAdapterService_GetHealthStatusClient,
// 	error) {
// 	if service == nil {
// 		return nil, nil, nil, nil, errors.New("service-nil")
// 	}
// 	adapterSvcClient, ok := service.(adapter_service.AdapterServiceClient)
// 	if ok {
// 		stream, err := adapterSvcClient.GetHealthStatus(ctx)
// 		if err != nil {
// 			logger.Errorw(ctx, "invalid-client", log.Fields{"client": adapterSvcClient, "service": reflect.TypeOf(service)})
// 		}
// 		return stream, nil, nil, nil, err
// 	}
// 	coreSvcClient, ok := service.(core_service.CoreServiceClient)
// 	if ok {
// 		stream, err := coreSvcClient.GetHealthStatus(ctx)
// 		if err != nil {
// 			logger.Errorw(ctx, "invalid-client", log.Fields{"client": adapterSvcClient, "service": reflect.TypeOf(service)})
// 		}
// 		return nil, stream, nil, nil, err
// 	}
// 	onuInterAdapterSvcClient, ok := service.(onu_inter_adapter_service.OnuInterAdapterServiceClient)
// 	if ok {
// 		stream, err := onuInterAdapterSvcClient.GetHealthStatus(ctx)
// 		if err != nil {
// 			logger.Errorw(ctx, "invalid-client", log.Fields{"client": adapterSvcClient, "service": reflect.TypeOf(service)})
// 		}
// 		return nil, nil, stream, nil, err
// 	}
// 	oltInterAdapterSvcClient, ok := service.(olt_inter_adapter_service.OltInterAdapterServiceClient)
// 	if ok {
// 		stream, err := oltInterAdapterSvcClient.GetHealthStatus(ctx)
// 		if err != nil {
// 			logger.Errorw(ctx, "invalid-client", log.Fields{"client": adapterSvcClient, "service": reflect.TypeOf(service)})
// 		}
...
// 	}
// 	return nil, nil, nil, nil, fmt.Errorf("invalid service %s", reflect.TypeOf(service))
// }
...
// func (c *Client) sendKeepAliveMessage(ctx context.Context,
// 	clientInfo *common.Connection,
// 	adapterSrvStream adapter_service.AdapterService_GetHealthStatusClient,
// 	coreSrvStream core_service.CoreService_GetHealthStatusClient,
// 	onuInterAdapterSrvStream onu_inter_adapter_service.OnuInterAdapterService_GetHealthStatusClient,
// 	oltInterAdapterSrvStream olt_inter_adapter_service.OltInterAdapterService_GetHealthStatusClient) error {

// 	serverHealth := &health.HealthStatus{}
// 	if adapterSrvStream != nil {
// 		return c.executeWithTimeout(func(conn *common.Connection) error { return adapterSrvStream.Send(conn) },
// 			func() (*health.HealthStatus, error) { return adapterSrvStream.Recv() },
// 			clientInfo,
// 			serverHealth,
// 			c.monitorInterval)
// 	}
// 	if coreSrvStream != nil {
// 		return c.executeWithTimeout(func(conn *common.Connection) error { return coreSrvStream.SendMsg(conn) },
// 			func() (*health.HealthStatus, error) { return coreSrvStream.Recv() },
// 			clientInfo,
// 			serverHealth,
// 			c.monitorInterval)
// 	}
// 	if onuInterAdapterSrvStream != nil {
// 		return c.executeWithTimeout(func(conn *common.Connection) error { return onuInterAdapterSrvStream.SendMsg(conn) },
// 			func() (*health.HealthStatus, error) { return onuInterAdapterSrvStream.Recv() },
// 			clientInfo,
// 			serverHealth,
// 			c.monitorInterval)
// 	}
// 	if oltInterAdapterSrvStream != nil {
// 		return c.executeWithTimeout(func(conn *common.Connection) error { return oltInterAdapterSrvStream.SendMsg(conn) },
// 			func() (*health.HealthStatus, error) { return oltInterAdapterSrvStream.Recv() },
// 			clientInfo,
// 			serverHealth,
// 			c.monitorInterval)
// 	}
// 	return errors.New("invalid streams nil")
// }
...
// func (c *Client) monitorConnection(ctx context.Context, handler GetServiceClient) {
// 	logger.Debugw(ctx, "monitor-connection-started", log.Fields{"qpi-endpoint": c.serverEndPoint, "client": c.clientEndpoint})
...
// 	srvClient := handler(ctx, conn)
// 	if srvClient == nil {
// 		logger.Errorw(ctx, "service-nil", log.Fields{"api-endpoint": c.serverEndPoint, "client": c.clientEndpoint})
// 		return
// 	}
...
// 	streamCtx, streamDone := context.WithCancel(log.WithSpanFromContext(context.Background(), ctx))
// 	defer streamDone()
// 	adapterSvcStream, coreSrvStream, onuInterAdapterStream, oltInterAdapterStream, err := getKeepAliveStream(streamCtx, srvClient)
// 	if err != nil {
// 		logger.Errorw(ctx, "cannot-create-a-stream", log.Fields{"api-endpoint": c.serverEndPoint, "client": c.clientEndpoint})
// 		return
// 	}
...
// 	for {
// 		// Let's send a keep alive message with our info
// 		err := c.sendKeepAliveMessage(ctx, clientInfo, adapterSvcStream, coreSrvStream, onuInterAdapterStream, oltInterAdapterStream)
// 		if err != nil {
// 			// Any error means the far end is gone
// 			logger.Errorw(ctx, "sending-stream-error", log.Fields{"error": err, "api-endpoint": c.serverEndPoint, "client": c.clientEndpoint})
// 			break loop
// 		}
// 		// Send a connect event
// 		if initialConnection {
...
// 		// Wait to send the next keep alive
// 		keepAliveTimer := time.NewTimer(time.Duration(clientInfo.KeepAliveInterval))
// 		// Check context of each stream one at a time - ugly!!!!
// 		if adapterSvcStream != nil {
// 			select {
// 			case <-ctx.Done():
// 				logger.Warnw(ctx, "context-done", log.Fields{"api-endpont": c.serverEndPoint, "client": c.clientEndpoint})
// 				break loop
// 			case <-adapterSvcStream.Context().Done():
// 				logger.Debugw(ctx, "stream-context-done", log.Fields{"api-endpoint": c.serverEndPoint, "stream-info": adapterSvcStream.Context(), "client": c.clientEndpoint})
// 				break loop
// 			case <-keepAliveTimer.C:
// 				continue
// 			}
...
// 				logger.Warnw(ctx, "context-done", log.Fields{"api-endpont": c.serverEndPoint, "client": c.clientEndpoint})
// 				break loop
// 			case <-coreSrvStream.Context().Done():
// 				logger.Debugw(ctx, "stream-context-done", log.Fields{"api-endpoint": c.serverEndPoint, "stream-info": coreSrvStream.Context(), "client": c.clientEndpoint})
// 				break loop
// 			case <-keepAliveTimer.C:
// 				continue
// 			}
// 		}
// 		if onuInterAdapterStream != nil {
// 			select {
// 			case <-ctx.Done():
// 				logger.Warnw(ctx, "context-done", log.Fields{"api-endpont": c.serverEndPoint, "client": c.clientEndpoint})
// 				break loop
// 			case <-onuInterAdapterStream.Context().Done():
// 				logger.Debugw(ctx, "stream-context-done", log.Fields{"api-endpoint": c.serverEndPoint, "stream-info": onuInterAdapterStream.Context(), "client": c.clientEndpoint})
// 				break loop
// 			case <-keepAliveTimer.C:
// 				continue
// 			}
// 		}
// 		if oltInterAdapterStream != nil {
// 			select {
// 			case <-ctx.Done():
// 				logger.Warnw(ctx, "context-done", log.Fields{"api-endpont": c.serverEndPoint, "client": c.clientEndpoint})
// 				break loop
// 			case <-oltInterAdapterStream.Context().Done():
// 				logger.Debugw(ctx, "stream-context-done", log.Fields{"api-endpoint": c.serverEndPoint, "stream-info": oltInterAdapterStream.Context(), "client": c.clientEndpoint})
// 				break loop
// 			case <-keepAliveTimer.C:
// 				continue
// 			}
...
// 	}
// 	if adapterSvcStream != nil {
// 		e := adapterSvcStream.CloseSend()
// 		logger.Debugw(ctx, "leaving", log.Fields{"api-endpoint": c.serverEndPoint, "error": e})
// 	}
// 	if coreSrvStream != nil {
// 		e := coreSrvStream.CloseSend()
// 		logger.Debugw(ctx, "leaving", log.Fields{"api-endpoint": c.serverEndPoint, "error": e})
// 	}
// 	if onuInterAdapterStream != nil {
// 		e := onuInterAdapterStream.CloseSend()
// 		logger.Debugw(ctx, "leaving", log.Fields{"api-endpoint": c.serverEndPoint, "error": e})
// 	}
// 	if oltInterAdapterStream != nil {
// 		e := oltInterAdapterStream.CloseSend()
// 		logger.Debugw(ctx, "leaving", log.Fields{"api-endpoint": c.serverEndPoint, "error": e})
// 	}
// }
