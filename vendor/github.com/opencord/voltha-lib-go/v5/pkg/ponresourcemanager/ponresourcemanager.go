...
const (
	...
	*/
	...
	ONU_GEM_INFO_PATH = "{%s}/onu_gem_info/{%d}/{%d}" // onu_gem/<intfid>/<onuID>
...
func SetKVClient(ctx context.Context, Technology string, Backend string, Addr string, configClient bool, basePathKvStore string) *db.Backend {
	...
	return db.NewBackend(ctx, Backend, Addr, KVSTORE_RETRY_TIMEOUT, pathPrefix)
...
func (PONRMgr *PONResourceManager) AddOnuGemInfo(ctx context.Context, intfID uint32, onuID uint32, onuGemData interface{}) error {
	/*
	...
	*/
	var Value []byte
	var err error
	Path := fmt.Sprintf(ONU_GEM_INFO_PATH, PONRMgr.DeviceID, intfID, onuID)
...
func (PONRMgr *PONResourceManager) GetOnuGemInfo(ctx context.Context, IntfId uint32, onuID uint32, onuGemInfo interface{}) error {
	/*
...
func (PONRMgr *PONResourceManager) DelOnuGemInfoForOnu(ctx context.Context, intfId uint32, onuID uint32) error {
	/*
